...
- **Working-paper status**: put the journal status of a `<wp>` in `<status>` (e.g. `Conditionally accepted at <em>Journal</em>.`, `Revisions requested by ...`, `Currently revising.`; CDATA when it contains HTML). It renders in bold before the notes on the research page, and after the date in the "Recent working papers" widget on the home page. The CV prints it in plain text and only when it names a journal (contains `<em>`), so "Currently revising" stays on the site only. Keep `<notes>` for everything else (NBER/IZA numbers, earlier titles).
